...
const isHtml = f => f.ext === '.html' || f.ext === '.htm';
const isScript = f => ['.js', '.mjs', '.cjs', '.jsx', '.ts', '.tsx'].includes(f.ext);
...
const SECRET_PATTERNS = [
  { re: /AKIA[0-9A-Z]{16}/g, what: 'AWS access key id' },
  { re: /ASIA[0-9A-Z]{16}/g, what: 'AWS temporary access key id' },
  { re: /aws_secret_access_key\s*=\s*\S+/gi, what: 'AWS secret access key' },
  { re: /-----BEGIN (?:RSA |EC |OPENSSH |PGP )?PRIVATE KEY-----/g, what: 'private key' },
  { re: /xox[baprs]-[A-Za-z0-9-]{10,}/g, what: 'Slack token' },
  { re: /gh[pousr]_[A-Za-z0-9]{20,}/g, what: 'GitHub token' },
  { re: /sk-[A-Za-z0-9]{32,}/g, what: 'OpenAI-style API key' },
  { re: /sk-ant-[A-Za-z0-9_-]{20,}/g, what: 'Anthropic API key' },
  { re: /eyJ[A-Za-z0-9_-]{10,}\.[A-Za-z0-9_-]{10,}\.[A-Za-z0-9_-]{10,}/g, what: 'JWT' },
  { re: /(?:postgres|mysql|mongodb(?:\+srv)?):\/\/[^\s:@/]+:[^\s:@/]+@/gi, what: 'database URL with inline credentials' },
];

/* Well-known public test credentials. These are published in vendor docs, are
   byte-identical in every local install, and are only valid against a loopback
   service — so reporting them as leaked credentials is a false positive that
   fires on every developer machine. A scanner that cries wolf on standard local
   tooling gets ignored, which costs more than it catches. */
function isKnownPublicTestCredential(match, content, index) {
  // Supabase local demo keys: payload is {"iss":"supabase-demo", ...}
  if (/^eyJ/.test(match)) {
    try {
      const payload = match.split('.')[1];
      const decoded = Buffer.from(payload.replace(/-/g, '+').replace(/_/g, '/'), 'base64').toString('utf8');
      if (/"iss"\s*:\s*"supabase-demo"/.test(decoded)) return true;
    } catch { /* not decodable — treat as a real finding */ }
  }
  // Stripe and similar vendors publish explicit test-mode prefixes.
  if (/^sk-(?:test|example)/i.test(match)) return true;
  return false;
}
  ...
  for (const file of files) {
    ...
    for (const { re, what } of SECRET_PATTERNS) {
      ...
      while ((m = re.exec(file.content))) {
        ...
        if (/YOUR_|EXAMPLE|PLACEHOLDER|xxxx|1234567890ABC/i.test(m[0])) continue;
        if (isKnownPublicTestCredential(m[0], file.content, m.index)) continue;
        hits++;
        report.add({
          id: 'CODE-03',
          title: `Possible ${what} committed to the repository`,
          severity: 'critical',
          confidence: CONFIDENCE.LIKELY,
          where: `${file.rel}:${lineOf(file.content, m.index)}`,
          detail: `Matched ${what}. Anything committed must be treated as disclosed, even after deletion — git history keeps it.`,
          remediation: 'Rotate the credential now, then remove it from history and move it to a secret store.',
        });
      }
    }
  }
